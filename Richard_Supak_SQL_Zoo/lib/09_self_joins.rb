...
def cl_to_lr_by_name
  ...
  execute(<<-SQL)
    ...
    WHERE stopa.name LIKE 'Craiglockhart' AND stopb.name LIKE 'London Road'
  SQL
end
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    SELECT start_routes.company, start_routes.num
    FROM routes AS start_routes
    JOIN routes AS end_routes ON start_routes.company = end_routes.company
      AND start_routes.num = end_routes.num
    JOIN stops AS origin_stops ON origin_stops.id = start_routes.stop_id
    JOIN 
